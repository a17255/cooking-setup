...
**Goal:** Add a floating Gemini-powered chatbot (bottom-right bubble) to the GitHub Pages cooking site that lets users search meals, get recommendations, re-roll suggestions, and more in Vietnamese or English.

**Architecture:** A standalone `docs/chatbot.js` file is injected via `<script src="chatbot.js">` in both `html_report.py` (template) and `docs/index.html` (current build). The chatbot reads the page's `allMeals` variable (accessible by name in subsequent scripts), sends the full meal list to Gemini 2.0 Flash in a system prompt, and executes structured JSON actions returned by the model.

**Tech Stack:** Vanilla JavaScript (no framework), Google Gemini API (`gemini-2.0-flash`, free tier), `localStorage` for API key + added dishes persistence, CSS injected via `document.head`.
...
|---|---|---|
| `docs/chatbot.js` | **CREATE** | All chatbot logic: UI, Gemini client, action handlers |
| `html_report.py` | **MODIFY** | Inject `<script src="chatbot.js">` in template (line ~473) |
| `docs/index.html` | **MODIFY** | Same script tag injection (line ~639) for immediate effect |
...
**Files:**
- Modify: `.gitignore`
- Modify: `html_report.py` (line 473–474)
- Modify: `docs/index.html` (line 639–640)

- [ ] **Step 1: Add `.superpowers/` to `.gitignore`**

Open `.gitignore` (or create it at project root if missing). Add at the end:
```
.superpowers/
```

- [ ] **Step 2: Inject script tag into `html_report.py`**

In `html_report.py`, find the line that reads `  </body>` (around line 474). Insert the script tag on the line immediately before it:
...
- [ ] **Step 3: Inject script tag into `docs/index.html`**

In `docs/index.html`, find `</body>` (line ~640). Insert before it:
...
- [ ] **Step 4: Create empty `docs/chatbot.js` to verify load**

Create `docs/chatbot.js` with this content:
```javascript
(function () {
...
Open `docs/index.html` in a browser. Open DevTools Console. Confirm you see:
```
[chatbot] loaded
```
...
```bash
git add .gitignore html_report.py docs/index.html docs/chatbot.js
git commit -m "feat: scaffold chatbot.js injection"
```

---

## Task 2: Chat UI — floating button + panel

**Files:**
- Modify: `docs/chatbot.js`

- [ ] **Step 1: Replace chatbot.js with UI skeleton**

Replace the full content of `docs/chatbot.js` with:
...
- [ ] **Step 2: Open `docs/index.html` in browser and verify**

Open `docs/index.html` directly in a browser (File → Open, or `start docs/index.html`).

Expected:
- Gold 💬 button appears bottom-right
- Clicking it opens the panel
- If no API key: setup screen shows (input + link)
- If key exists: welcome message shows
- ✕ closes the panel
- ⚙ re-opens setup screen

- [ ] **Step 3: Commit**

```bash
git add docs/chatbot.js
git commit -m "feat: add chatbot UI, Gemini client, and all action handlers"
```
...
Prerequisites: get a free Gemini API key at https://aistudio.google.com/app/apikey

- [ ] **Step 1: Enter API key**

Open `docs/index.html` in browser. Click 💬, enter your Gemini API key, click "Lưu key".
...
Expected: bot replies with a list of dishes containing "sưởn" from your meal list, in Vietnamese.

- [ ] **Step 3: Test count**

Type: `có bao nhiêu món chính?`

Expected: bot replies with the correct count of main dishes.

- [ ] **Step 4: Test recommendation**

Type: `gợi ý món nhẹ tối nay`

Expected: bot picks a specific dish and explains why.

- [ ] **Step 5: Test re-roll via chat**

Type: `đổi món chính khác đi`

Expected: bot replies confirming the change AND the "Main" card on the page updates to a new dish.

- [ ] **Step 6: Test full re-roll**

Type: `đổi toàn bộ thực đơn`

Expected: all three cards change and bot confirms.

- [ ] **Step 7: Test dish explanation**

Type: `bánh canh là gì?`

Expected: bot explains the dish.

- [ ] **Step 8: Test add dish**

Type: `thêm bún bò Huế vào món chính`

Expected: bot confirms it added "Bún bò Huế" to main dishes. Verify by then typing `có bao nhiêu món chính?` — count should increase by 1. After page refresh, type again — added dish should still be there (localStorage persisted).

- [ ] **Step 9: Test weekly plan**

Type: `lên thực đơn 5 ngày cho gia đình`

Expected: bot returns a formatted 5-day plan using dishes from the list.

- [ ] **Step 10: Test today's menu**

Type: `hôm nay ăn gì?`

Expected: bot reads the current page cards and describes today's menu.

- [ ] **Step 11: Test English**

Type: `how many soups are there?`

Expected: bot replies in English with the correct count.

- [ ] **Step 12: Test error handling — bad key**

Open ⚙, enter a fake key like `FAKE123`, save. Type any message.

Expected: error message "API key không hợp lệ. Nhấn ⚙ để cập nhật."

Restore your real key via ⚙ before proceeding.

- [ ] **Step 13: Final commit**

```bash
git add docs/chatbot.js docs/index.html html_report.py .gitignore
git commit -m "feat: add Gemini chatbot to cooking site"
```
...
Open your GitHub Pages URL. Confirm the 💬 button appears and the chatbot works on the live site.

Note: your Gemini API key is stored only in your browser's localStorage — it is never committed to the repo.
